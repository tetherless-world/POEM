	...
	<xsl:template match="//dataroot" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		<html>
			<head>
				...
				<title>04_EntityGroups</title>
				<style type="text/css"></style>
			</head>
			<body link="#0c0000" vlink="#050000">
				<table border="1" bgcolor="#ffffff" cellspacing="0" cellpadding="0" id="CTRL1">
					<colgroup>
						<col style="WIDTH: 1.3645in"/>
						<col style="WIDTH: 4.1458in"/>
					</colgroup>
					<tbody>
						<tr>
							<td>
								<div align="center">
									<strong>EntityMappingID</strong>
								</div>
							</td>
							<td>
								<div align="center">
									<strong>EntityListName</strong>
								</div>
							</td>
						</tr>
					</tbody>
					<tbody id="CTRL2">
						<xsl:for-each select="_x0030_4_EntityGroups">
							<!-- Cache the current node incase the a field is formatted -->
							<xsl:value-of select="fx:CacheCurrentNode(.)"/>
							<tr>
								<td>
									<xsl:value-of select="EntityMappingID"/>
								</td>
								<td>
									<xsl:value-of select="EntityListName"/>
								</td>
							</tr>
						</xsl:for-each>
					</tbody>
